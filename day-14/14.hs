...
day_14b = do
    ...
    let conversions = lines_to_conversions (lines text)
...
lines_to_conversions :: [String] -> [Conversion]
lines_to_conversions lns = let 
    split_1 = map (map (Split.splitOn ", ")) (map (Split.splitOn " => ") lns)
    split_2 = map (map item_to_tup) (map (concat) split_1)
    in map to_conversion split_2
...
item_to_tup item = (s !! 1, read (s !! 0) :: Int) where s = Split.splitOn " " item
...
calc all_conversions requested_ingredients leftovers
    ...
    | otherwise = {-trace ("requested: " ++ show new_requested_ingredients ++ "-------  leftovers: " ++ (show leftovers)) $ -} calc all_conversions new_requested_ingredients (fst new_status)
        where
            filtered_requests_list = Map.toList $ Map.filterWithKey (\k _ -> k /= "ORE" ) requested_ingredients -- get all entries that are not ore
            ore_requests_list = Map.toList $ Map.filterWithKey (\k _ -> k == "ORE" ) requested_ingredients -- get all entries that are ore
