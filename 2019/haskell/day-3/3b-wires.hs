-- The key to this solution is to run 3a without sorting the intersections. The first intersection that appears is the intersection we know. 
-- Because I simply put every step in the lists we can then just count the indices.
...
-- get new coordinates after applying a single wire from a starting location
add_wire :: (Int, Int) -> [Char] -> (Int, Int) 
add_wire loc wire = let val = read (tail wire) :: Int
    in case head wire of 
        'R' -> ((fst loc) + val, snd loc)
        'D' -> (fst loc, (snd loc) - val)
        'U' -> (fst loc, (snd loc) + val)
        'L' -> ((fst loc) - val, snd loc)
                            
-- get all locations from a wire
get_corners :: [[Char]] -> [(Int, Int)] 
get_corners wires = scanl (add_wire) (0,0) wires

locs_1 = get_corners wires_1 
locs_2 = get_corners wires_2 

all_locations corners = foldl get_range [(0,0)] corners

-- append a range from the last element of the current list to the next corner. 
-- The tail functions are used to prevent duplicates
get_range full_list corner 
    | (last full_list) > corner = full_list ++ tail (reverse (range (corner, (last full_list)))) -- this is because ranges always count to positive
    | otherwise = full_list ++ tail(range ((last full_list), corner))

intersections = intersect (all_locations locs_1) (all_locations locs_2)

-- get manhattan distance of tuple
